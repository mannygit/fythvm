...
- Source URL: https://compilercrim.es/bootstrap/miniforth/
- Fetched date: 2026-04-15

This article is part of the [Bootstrapping](https://compilercrim.es/bootstrap/)
...
just downloading it. How do you break this cycle?[1](#cycle) Since the [bootstrapping
problem](http://bootstrappable.org/) has first come to my attention, I've been drawn to
this unique area of software engineering. Not out of fear that someone would
try to implement a [trusting trust](http://users.ece.cmu.edu/~ganger/712.fall02/papers/p761-thompson.pdf) attack, but simply as an interesting
challenge.

11 years ago, [*vanjos72* described on Reddit](https://www.reddit.com/r/programming/comments/9x15g/programming_thought_experiment_stuck_in_a_room/) what he
...
conversion code.[2](#ascii-x86) Moreover, the loop doesn't even need an end
...
bootstrap seed that fits into the bootsector is fair game.[3](#disagree) Obviously,
...
such as [a DooM-like raycasting game](https://github.com/nanochess/cubicDoom) or a [chess AI](https://github.com/nanochess/Toledo-Atomchess),
as well as a basic [BASIC interpreter](https://github.com/nanochess/bootBASIC), but
perhaps the most relevant one for our usecase is [bootOS](https://github.com/nanochess/bootOS):
...
GitHub](https://github.com/meithecatte/miniforth/tree/post1), but I will include most of it here.
...
optimized code, [Ilya Kurdyukov](https://twitter.com/ilyakurdyukov) came along and managed to find 24 bytes
to be saved! I promptly reinvested this saved space in [new
features](https://compilercrim.es/bootstrap/miniforth/#s-string-poke).
...
spectacularily](https://twitter.com/meithecatte/status/1401557309118103560) because of my inability to actually pick
a register that can be safely overwritten.

Ilya Kurdyukov then demonstrated [that the same bytecount can be achieved
without this kind of "magic"](https://twitter.com/ilyakurdyukov/status/1401736488681979906). An analogous modification allowed
...
The first thing done after booting is setting up the [segment registers](https://en.wikipedia.org/wiki/X86_memory_segmentation) and
...
x86 architecture. To quote the Volume 2B of the [x86 Software Developer's Manual](https://software.intel.com/content/www/us/en/develop/articles/intel-sdm.html):

> Loading the SS register with a POP instruction[5](#pop-ss) suppresses or inhibits some debug exceptions and inhibits interrupts on the following instruction boundary. (The inhibition ends after delivery of an exception or the execution of the next instruction.) This behavior allows a stack pointer to be loaded into the ESP register with the next instruction (POP ESP)[6](#pop-esp) before an event can be delivered.
...
`0x500`, directly after the [BDA](https://www.matrix-bios.nl/system/bda.html). While the idiomatic string format for Forth
...
Finally, [Ralf Brown's Interrupt List](http://www.ctyme.com/rbrown.htm) mentions that some BIOSes clobber
...
width to be 2 bytes wide.[8](#base-dw)
...
to implement any stack manipulation word you can imagine.[9](#stack-complete)
...
so I'll be using it from now on.[10](#future)

[A photo of the corner of a monitor. We see the calculation "6969 4242 100 + + u-dot", which results in "ACAB". Then the "1 load" command is issued, to which the system describes "Hello, world!"]

If you'd like to see how to bootstrap on top of Miniforth's minimal set of
primitives, see the [next post in this series](https://compilercrim.es/bootstrap/branches/).

# Hope you enjoyed this!

You might like some of [my other posts](/#blog), too. If you'd like to be notified
of new ones, you can [follow me on the Fediverse](https://donotsta.re/mei)
or subscribe to the [RSS feed](/rss.xml).

Also, I'd like to thank
[my GitHub sponsors](https://github.com/sponsors/meithecatte) for their support:
...
I would wager that the typical system involves a tangled strongly
connected component of dependencies, rather than just a simple cycle.

2

And even if that wasn't the case, there are [many](https://en.wikipedia.org/wiki/EICAR_test_file#Design),
[many](http://tom7.org/abc/)
[examples](https://github.com/pts/pts-xcom) of x86 code written with the printable subset of ASCII.
I've even [done it myself](https://codegolf.stackexchange.com/questions/155018/the-programming-language-quiz-mark-ii-cops/155023#155023) once a few years ago.

3

If you, dear reader, find this unsatisfactory, I would like to
...
reader](https://abstrusegoose.com/12).[11](#exercise)

10

A note from the future: I've since switched to dual-booting it on my
laptop.

11

Seriously though, try to devise an infallible strategy for turning
...
[[Image]](https://diyhrt.wiki)

[[The first third of a 88x31 with a glider and arrow going to
the left]](//ring.acab.dev/prev/13e80a04a7)
[[The middle third of a 88x31 with the text Hacker Webring on
it. In the background of the text is a ring with an offset shadow.]](//ring.acab.dev)
[[The last third of a 88x31 with a glider and arrow going to the
right]](//ring.acab.dev/next/13e80a04a7)
