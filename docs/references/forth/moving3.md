...
Source URL: https://www.bradrodriguez.com/papers/moving3.htm
...
a. it compiles the Forth word (;CODE) into CONSTANT, b. it turns off the Forth compiler, and c. it turns on the Forth assembler.
...
b. It puts that address into the Code Field of the word just defined by CREATE. CREATE remembers the address of the last word created in a variable called LAST (or LATEST). To change the Code Field, all (;CODE) has to do is get the CFA of the word named by LAST, and store the address into it.

c. It performs the action of EXIT, causing CONSTANT to terminate here and not attempt to execute the machine code fragment that follows.

In F83, for instance, (;CODE) is
...
Walk through this code to see how the machine code address is obtained from the Return Stack, and put into the Code Field of the most recently defined word. Remember that in Forth, if you omit the final ; from a high-level colon definition, then the word ends with an implicit EXIT -- which in F83 is simply the machine code instruction RET. So, ;CODE is not as mysterious as it first appears.

## HIGH-LEVEL ACTIONS

The machine-code fragments shown so far are straightforward. Now let's do something more interesting: define a new class of Forth words whose action is itself a high-level Forth routine. This is what DOES> does.

Suppose we want to write a defining word that creates words like these:
...
and later attach a shared high-level action to them. The classic example is a defining word such as CONSTANT written using DOES> rather than ;CODE.

To make this work, a machine language routine, or a set of routines, is needed to handle the problems of invoking a high- level action. We'll call this routine DODOES. There are three problems to be solved:
...
a. It compiles the Forth word (DOES>) into CONSTANT. b. It compiles a JSR DODOES into CONSTANT.
...
Continue with Part 4 | Back to publications page
